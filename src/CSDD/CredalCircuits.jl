...
"""
function log_likelihood_upper_per_instance(pc::CredalΔ, batch::PlainXData{Bool})
    fc = FlowΔ(pc, num_examples(batch), Bool)
    (fc, log_likelihood_upper_per_instance(fc, batch))
end

function log_likelihood_lower_per_instance(pc::CredalΔ, batch::PlainXData{Bool})
    fc = FlowΔ(pc, num_examples(batch), Bool)
    (fc, log_likelihood_lower_per_instance(fc, batch))
end
"""
...
function log_prob_upper(pc::CredalΔ, batch::PlainXData{Bool})
    log_likelihood_upper_per_instance(pc, batch)[2]
end


function log_prob_lower(pc::CredalΔ, batch::PlainXData{Bool})
    log_likelihood_lower_per_instance(pc, batch)[2]
end

function log_marginal_upper(pc::CredalΔ, batch::PlainXData{Int8})
    marginal_log_likelihood_upper_per_instance(pc, batch)[2]
end


function log_marginal_lower(pc::CredalΔ, batch::PlainXData{Int8})
    marginal_log_likelihood_lower_per_instance(pc, batch)[2]
end
...
"""
Calculate the lower log likelihood for a batch of samples with partial evidence P(e).
(Also returns the generated FlowΔ)

To indicate a variable is not observed, pass -1 for that variable.
"""
function marginal_log_likelihood_lower_per_instance(pc::CredalΔ, batch::PlainXData{Int8})
    opts = (flow_opts★..., el_type=Float64, compact⋀=false, compact⋁=false)
    fc = UpFlowΔ(pc, num_examples(batch), Float64, opts)
    (fc, marginal_log_likelihood_lower_per_instance(fc, batch))
end
...
To indicate a variable is not observed, pass -1 for that variable.
"""
function marginal_log_likelihood_lower_per_instance(fc::UpFlowΔ, batch::PlainXData{Int8})
    @assert (prob_origin(fc[end]) isa CredalΔNode) "FlowΔ must originate in a CredalΔ"
